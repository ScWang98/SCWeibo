...
@objc protocol EmojiToolBarDelegate: NSObjectProtocol {
    /// 表情工具栏分组选择
    /// - Parameters:
    ///   - tooBar: EmojiToolbar
    ///   - index: 点击的索引下标
...
class EmojiToolbar: UIView {
    weak var delegate: EmojiToolBarDelegate?
...
private extension EmojiToolbar {
    func setupUI() {
        for (index, package) in EmojiManager.shared.packages.enumerated() {
            let btn = UIButton()
            ...
            var normalImage = UIImage(named: normalImageName, in: EmojiManager.shared.bundle, compatibleWith: nil)
            var selectedImage = UIImage(named: selectedImageName, in: EmojiManager.shared.bundle, compatibleWith: nil)
